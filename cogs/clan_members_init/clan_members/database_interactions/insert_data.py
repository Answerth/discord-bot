...
from sqlalchemy.dialects.postgresql import insert  # Import PostgreSQL-specific insert
            ...
            try:
                if table_name == "members" and unique_column:
                    ...
                    if result.rowcount == 0:
                        log_conflict(f"Conflict skipped for activity row", row.to_dict())
                    else:
...
# Upsert in 'members' to handle updates if any value changes
insert_data(engine, members_df, 'members', unique_column="name")
# Insert all records in 'activities' without conflict handling
